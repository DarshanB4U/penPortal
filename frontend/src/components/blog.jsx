...
export function Blog({ authername, publishdate, title, content }) {
  ...
  return (
    <div className="flex justify-center ">
      <div className="m-3 p-5 border-orange-600  hover:shadow-md shadow-rose-400  border-b max-w-2xl  ">
        <div className="flex gap-1  justify-start font-serif text-md  font-medium">
          <Avatar name={authername} />{" "}
          <div className="p-2">
            {authername} . {publishdate}
          </div>
        </div>
        <div className="text-2xl py-1 font-bold">{title}</div>
        <div>{content}</div>
...
export function Avatar({ name }) {
  return (
    <div className="relative inline-flex items-center justify-center w-10 h-10 overflow-hidden hover:bg-green-800  bg-slate-800 rounded-full dark:bg-slate-800">
